...
export async function analyzeImageQuality(imageElement) {
  ...
  const uniformity = Math.max(0, 100 - Math.sqrt(variance)); // higher = more uniform surface
  const avgSaturation = (satSum / n) * 100;

  // Simple scoring: reward uniform surface + healthy brightness range + reasonable saturation.
  const brightnessScore = avgBrightness > 60 && avgBrightness < 220 ? 100 : 60;
  const score = Math.round(uniformity * 0.5 + brightnessScore * 0.3 + Math.min(avgSaturation, 100) * 0.2);

  let grade, reasons;
  if (score >= 75) {
    grade = "A";
    reasons = ["Surface uniform dikh raha hai", "Rang healthy range mein hai", "Koi bada daag/defect nahi dikha"];
  } else if (score >= 55) {
    grade = "B";
    reasons = ["Surface mein halka farak hai", "Kuch hisson mein rang asaman hai", "Overall theek-thaak quality"];
  } else {
    grade = "C";
    reasons = ["Surface mein zyada variation hai", "Roshni/angle sahi nahi tha ya upaj mein daag ho sakte hain", "Behtar photo (saaf background, achhi roshni) dobara try karein"];
  }
