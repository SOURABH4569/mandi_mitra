...
export default function MandiList({ mandis, farmerLocation, qty, onSelect }) {
  ...
  const enriched = mandis
    .map((m) => ({
      ...
      demandStatus:
        typeof m.arrivalVolume === "number"
          ? getDemandSupplyStatus(m, avgArrival)
          : null,
    }))
    .sort(
      (a, b) =>
        b.econ.netPricePerQuintal - a.econ.netPricePerQuintal
    );
